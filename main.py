...
IMG_STEP2 = rf"{BASE}/SetUp/Paso2e.png"
...
class App:
    def __init__(self):
        ...
        Button(self.root, text="Registrar manualmente", command=self.open_register_form).place(x=1100, y=680)

        # Intentar abrir cámara externa (1), si falla usar interna (0)
        self.cap = None
        for cam_index in [1, 0]:
            ...
            if cap.isOpened():
                self.cap = cap
                print(f"Cámara iniciada en índice {cam_index}")
                break

        if self.cap is None:
            messagebox.showerror("Cámara", "No se detectó ninguna cámara disponible.\nConecte una webcam y reinicie el programa.")
            self.root.destroy()
            return
    ...
    def open_register_form(self):
        ...
        self.id_var = StringVar(value=gen_short_id())
    ...
    def loop(self):
        ...
        if self.state == "register_wait_blink":
            safe_overlay(frame_bgr, self.step0, 50, 50)
            safe_overlay(frame_bgr, self.step1, 50, 1030)
